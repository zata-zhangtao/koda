...
# Task Plan: Claude Runner Lint Bootstrap Fix

**Goal**: Restore post-review `pre-commit` execution for the Claude Code support task by removing runtime GitHub hook fetches from `.pre-commit-config.yaml` and verifying `uv run pre-commit run --all-files` succeeds inside this worktree.
**Started**: 2026-03-27
...
### Phase 1: Discovery
- [x] Confirm the latest lint failure happened before hook execution because `pre-commit` tried to fetch `pre-commit-hooks` from GitHub and hit a TLS handshake error
- [x] Verify the same remote-bootstrap pattern also affects `ruff-pre-commit`
- **Status:** complete

### Phase 2: Implementation
- [x] Replace GitHub-backed `pre-commit-hooks` and `ruff-pre-commit` entries with local hooks executed via the project environment
- [x] Declare `ruff` and `pre-commit-hooks` as dev dependencies so the local hook commands remain reproducible
- **Status:** complete

### Phase 3: Verification
- [x] Run `uv run pre-commit run --all-files` and absorb the first-pass `ruff-format` auto-fixes
- [x] Rerun `uv run pre-commit run --all-files` and confirm the full hook set passes
- **Status:** complete

## Decisions Made
| Decision | Rationale |
|----------|-----------|
| Localize hook execution through `uv run ...` instead of remote hook repositories | The blocker was GitHub TLS failure during hook bootstrap, so the narrowest reliable fix was to stop cloning GitHub hook repos during lint |
| Add `ruff` and `pre-commit-hooks` to `pyproject.toml` dev dependencies | The new local hook definitions depend on those packages, and the repo rules require project dependencies to be declared explicitly |

## Completion Summary
- **Status:** Complete (2026-03-27)
- **Tests:**
  - `uv add --dev ruff pre-commit-hooks` -> PASS
  - `uv run pre-commit run --all-files` -> FAIL first pass (`ruff-format` auto-fixed files)
  - `uv run pre-commit run --all-files` -> PASS second pass
- **PRD:** Updated `tasks/prd-3f0bc724.md`
- **Deliverables:**
  - `.pre-commit-config.yaml` - GitHub-backed hook repos replaced with local hooks that run from the project environment
  - `pyproject.toml`, `uv.lock` - declared and locked `ruff` plus `pre-commit-hooks` for reproducible lint execution
- **Notes:** `uv run` still emits a non-blocking `VIRTUAL_ENV` mismatch warning because the parent shell exports `/home/tao/codes/koda/.venv`; lint execution itself is now green.
