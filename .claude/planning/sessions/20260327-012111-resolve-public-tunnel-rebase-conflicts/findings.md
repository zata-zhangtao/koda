...
## 2026-03-27 Claude Runner Lint Bootstrap Findings
- The latest post-review lint failure occurred before any file checks ran because `.pre-commit-config.yaml` still referenced `https://github.com/pre-commit/pre-commit-hooks`, and `git fetch origin --tags` failed with `gnutls_handshake() failed`.
- The same configuration pattern also existed for `https://github.com/astral-sh/ruff-pre-commit`, so fixing only the first repo would leave the next remote hook bootstrap failure in place.
- The project environment already had `PyYAML`, but it did not have `ruff` or `pre-commit-hooks`, which meant switching to local hooks required explicit dev dependency declarations rather than ad hoc shell commands.
- Replacing the remote hook repos with `repo: local` entries that call `uv run ...` is sufficient to remove the GitHub bootstrap dependency while preserving the same lint surfaces (`trailing-whitespace`, `end-of-file-fixer`, `check-yaml`, `check-toml`, `check-added-large-files`, `debug-statements`, `ruff`, `ruff-format`).
- After the bootstrap fix, the only follow-up lint failure was the expected first-pass `ruff-format` rewrite; a second `uv run pre-commit run --all-files` completed successfully.
- The `VIRTUAL_ENV=/home/tao/codes/koda/.venv` mismatch warning still appears on every `uv run` invocation, but it is external environment noise rather than a `pre-commit` blocker.
