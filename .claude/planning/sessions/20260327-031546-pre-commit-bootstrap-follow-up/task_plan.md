# Task Plan: Pre-commit Bootstrap Follow-up

**Goal**: Remove the current lint blockers caused by deprecated pre-commit stage naming and GitHub-backed hook bootstrap failures, without broadening product scope beyond the reported `uv run pre-commit run --all-files` output.
**Started**: 2026-03-27

## Phases

### Phase 1: Discovery
- [x] Inspect the reported lint output and the current `.pre-commit-config.yaml` hook sources
- [x] Confirm whether the failure is limited to hook configuration/bootstrap instead of business-code lint violations
- **Status:** complete

### Phase 2: Implementation
- [x] Replace GitHub-backed `pre-commit-hooks` and `ruff-pre-commit` repositories with local `uv run ...` hook entries
- [x] Update the deprecated `archive-tasks` stage from `commit` to `pre-commit`
- [x] Add the required local CLI dependencies to the dev dependency group
- **Status:** complete

### Phase 3: Verification
- [x] Refresh `uv.lock` and the project environment after the dependency additions
- [x] Restore the original file-type filtering on the localized hooks so YAML/TOML/Python checks stay scoped correctly
- [x] Re-run `uv run pre-commit run --all-files` successfully and confirm the feature PRD needs no content change for this lint-only follow-up
- **Status:** complete
...
| Decision | Rationale |
|----------|-----------|
| Convert both remote hook repos to `repo: local` instead of changing URLs | The blocker is GitHub fetch failure during pre-commit bootstrap; local hooks remove that failure mode entirely and keep lint execution inside the project environment |
| Add `pre-commit-hooks` and `ruff` as dev dependencies | Local hooks still need stable executables; declaring them in `pyproject.toml` keeps setup reproducible under `uv sync` |
| Review `tasks/prd-627540a9.md` but leave it unchanged | This round only repairs lint infrastructure and does not alter the user-facing timeline feature contract |
...
- **PRD:** Reviewed `tasks/prd-627540a9.md` and left unchanged (lint/config-only follow-up)
- **Deliverables:**
  - `.pre-commit-config.yaml` - local hook bootstrap plus non-deprecated stage naming
  - `pyproject.toml`, `uv.lock` - declared and locked local hook dependencies

# Task Plan: Project Timeline Blocker Follow-up

**Goal**: Close the two blocking self-review gaps for the project timeline card: make historical-task references persistent and idempotent via a dedicated relation model, and expose a real user-facing `ABANDONED` action so the new lifecycle state is reachable instead of filter-only.
**Started**: 2026-03-27

## Phases

### Phase 1: Discovery
- [x] Re-read the blocking review findings and inspect the affected task-reference API, database bootstrap, App actions, and existing tests
- [x] Confirm the smallest data model and UI changes that satisfy the PRD without broadening scope
- **Status:** complete

### Phase 2: Implementation
- [x] Add persistent `task_reference_link` storage plus idempotent reference creation behavior
- [x] Add a reachable `ABANDONED` action in the main UI and preserve deleted vs abandoned semantics
- [x] Update tests and docs only where they directly cover these blockers
- **Status:** complete

### Phase 3: Verification
- [x] Run focused pytest coverage for project timeline/task reference/database regressions
- [x] Run frontend build and docs build
- [x] Sync the PRD with the delivered blocker fixes
- **Status:** complete
...
| Decision | Rationale |
|----------|-----------|
| Fix reference dedup with a dedicated `task_reference_links` table instead of heuristics alone | The PRD explicitly promised a persistent `source -> target` relation for audit and dedup; a table is the smallest reliable way to satisfy that contract |
| Keep `POST /api/tasks/{target_task_id}/references` backwards compatible and return the first reference log on duplicate calls | The frontend already calls this endpoint; idempotent reuse avoids unnecessary client changes while stopping duplicate timeline pollution |
| Treat `ABANDONED` as a first-class action parallel to `Delete` | The review blocker is specifically that the state exists only in enums and filters, so the narrowest fix is a reachable UI action rather than more backend plumbing |

## Completion Summary
- **Status:** Complete (2026-03-27)
- **Tests:**
  - `UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/test_project_timeline_api.py tests/test_project_service.py tests/test_database.py -q` -> PASS (`20 passed`)
  - `cd frontend && npm run build` -> PASS
  - `just docs-build` -> PASS
- **PRD:** Updated `tasks/prd-627540a9.md`
- **Deliverables:**
  - `dsl/models/task_reference_link.py`, `dsl/api/tasks.py`, `dsl/models/__init__.py`, `dsl/schemas/task_schema.py` - persistent task reference relation plus idempotent historical-task reference creation
  - `frontend/src/App.tsx` - reachable `Abandon` action and matching timeline system-event marker
  - `tests/test_project_timeline_api.py`, `tests/test_database.py` - regression coverage for task-reference dedup, UI reachability, and schema bootstrap
  - `docs/database/schema.md`, `docs/api/references.md`, `tasks/prd-627540a9.md` - synchronized data model/API/PRD documentation for the blocker fixes

## Blocker Follow-up
- **Status:** Complete (2026-03-27)
- **Scope:** Close the second-round self-review blocker on `POST /api/tasks/{target_task_id}/references` by protecting `requirement_brief` appends against the 5000-character column limit without broadening the feature scope.
- **Outcome:** Added preflight length checks plus compact-append fallback in `dsl/api/tasks.py`, centralized the column limit in `dsl/models/task.py`, documented the bounded behavior, and added regression tests for both compact fallback and fail-fast `422`.
- **Verification:**
  - `UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/test_project_timeline_api.py tests/test_project_service.py tests/test_database.py -q` -> PASS (`22 passed`)
  - `just docs-build` -> PASS
...
# Task Plan: Project Timeline By Project Category

**Goal**: Finish and verify the project-level timeline management feature so logs can be reviewed by project category (`Task.project_id -> Project`), including task status filtering, PRD/planning snapshots, historical task reference, frontend entry page, and synchronized docs/tests.
**Started**: 2026-03-27

## Phases

### Phase 1: Audit Existing Changes
- [x] Inspect the PRD for task `627540a9` and confirm the recommended interpretation of "按项目类别"
- [x] Review the already-modified backend/frontend files to see which parts of the feature are already implemented
- [x] Review the new project timeline tests to understand the intended contract
- **Status:** complete

### Phase 2: Verification And Gap Fixes
- [x] Run focused backend tests for project timeline, chronicle, and task reference behavior
- [x] Run frontend build and docs build to catch integration or documentation regressions
- [x] Fix the planning snapshot gap so timeline detail reads real planning files and persists them before worktree cleanup
- **Status:** complete

### Phase 3: Finalize Delivery
- [x] Re-run affected checks until green
- [x] Update PRD with delivered scope, verification evidence, and any deviations
- [x] Archive planning session after completion
- **Status:** complete
...
| Decision | Rationale |
|----------|-----------|
| Interpret “按项目类别” as grouping by existing `Project` relation instead of introducing a new taxonomy | This matches the task PRD recommendation and avoids inventing another classification model |
| Treat the current dirty worktree as in-progress user-approved feature work | Existing modified files already line up with the PRD; the safest path is verify and complete them rather than restart |
| Prioritize tests before further edits | The repository already contains new API/page/test surfaces, so running verification is the fastest way to find the real missing pieces |

## Completion Summary
- **Status:** Complete (2026-03-27)
- **Tests:** `pytest` focused suites, `py_compile`, `npm run build`, and `uv run mkdocs build --strict` all passing
- **PRD:** Updated `tasks/prd-627540a9.md`
- **Planning Archive:** `.claude/planning/sessions/20260327-011230-resolve-public-tunnel-rebase-conflicts`
# Task Plan: Project Category Timeline Management

**Goal**: Extend the existing project timeline feature so logs can be managed by project category, with durable project category metadata, category-scoped timeline filtering, and frontend controls that surface cross-project category history.
**Started**: 2026-03-27

## Phases

### Phase 1: Discovery
- [x] Inspect the existing project timeline API, service, schemas, frontend page, and project model
- [x] Confirm whether a persistent project-category field already exists
- [x] Decide the minimum compatible contract change needed to support category-scoped timeline browsing
- **Status:** complete

### Phase 2: Implementation
- [x] Add persistent `project_category` support to the project model, schemas, API responses, database bootstrap patches, and project management UI
- [x] Extend chronicle timeline/summary/detail payloads so the timeline page can browse tasks across project categories instead of a single project only
- [x] Update the timeline page to filter by project category, show project/category context per task, and keep history-reference actions coherent
- **Status:** complete

### Phase 3: Verification
- [x] Add regression coverage for project-category persistence and category-scoped timeline filtering
- [x] Run focused Python tests, frontend build, and `just docs-build`
- [x] Sync the active PRD with delivered behavior and verification evidence
- **Status:** complete
...
| Decision | Rationale |
|----------|-----------|
| Add a first-class nullable `Project.project_category` field instead of overloading `display_name` or `description` | The user asked for project-category-based timeline management, and no stable category dimension exists in the current model |
| Extend the existing `/api/chronicle/project-timeline` contract with optional category scope instead of creating a parallel second timeline API | This preserves the current page flow and keeps project-specific and category-scoped browsing on one path |
| Keep task detail retrieval task-based, but enrich timeline entries/details with project name and category metadata | The interaction model is still “select a task to inspect logs,” even when the list is filtered by category rather than a single project |

## Completion Summary
- **Status:** Complete (2026-03-27)
- **Tests:**
  - `uv run pytest tests/test_project_service.py tests/test_project_timeline_api.py tests/test_database.py -q` -> PASS (`13 passed`)
  - `cd frontend && npm run build` -> PASS
  - `just docs-build` -> PASS
- **PRD:** Updated `tasks/prd-627540a9.md`
- **Deliverables:**
  - `dsl/models/project.py`, `dsl/schemas/project_schema.py`, `dsl/services/project_service.py`, `dsl/api/projects.py`, `utils/database.py` - persistent `project_category` support plus SQLite compatibility patch
  - `dsl/services/chronicle_service.py`, `dsl/api/chronicle.py`, `dsl/schemas/chronicle_schema.py` - category-scoped project timeline queries, summary scope expansion, and project metadata on timeline payloads
  - `frontend/src/App.tsx`, `frontend/src/pages/ProjectTimelinePage.tsx`, `frontend/src/api/client.ts`, `frontend/src/types/index.ts`, `frontend/src/index.css` - project-category management UI plus category-aware timeline browsing
  - `tests/test_project_service.py`, `tests/test_project_timeline_api.py`, `tests/test_database.py`, `docs/database/schema.md`, `docs/guides/dsl-development.md`, `tasks/prd-627540a9.md` - regression coverage and synchronized documentation

## Follow-up Review Fixes (2026-03-27)
- Scope the project timeline date filter to in-window task/log activity instead of whole-task `last_activity_at`
- Restore `PENDING` to the shared default status filter used by the timeline page and summary request path
- Add regression coverage for date-window stats/summary semantics and the frontend default status contract
- **Status:** complete
- **Verification:**
  - `uv run pytest tests/test_project_timeline_api.py tests/test_project_service.py tests/test_database.py -q` -> PASS (`16 passed`)
  - `cd frontend && npm run build` -> PASS
  - `just docs-build` -> PASS
# Task Plan: Project Timeline Blocker Fixes

**Goal**: Resolve the two blocking self-review regressions in the project timeline feature by keeping summary milestones inside the requested date window and restoring scrolling on the standalone `/project-timeline` page, then verify with targeted regression coverage and sync the active PRD.
**Started**: 2026-03-27

## Phases

### Phase 1: Discovery
- [x] Re-read the self-review findings and inspect the affected summary and CSS code paths
- [x] Confirm the minimum regression tests needed for the date-window milestone bug and the page-scroll CSS contract
- **Status:** complete

### Phase 2: Implementation
- [ ] Gate closed-task milestone text on an in-window `closed_at` timestamp
- [ ] Add a dedicated scroll container rule for `/project-timeline`
- [ ] Add focused regression coverage for both blockers
- **Status:** in_progress

### Phase 3: Verification
- [ ] Run focused pytest coverage for timeline regressions
- [ ] Run the frontend production build
- [ ] Sync `tasks/prd-627540a9.md` with the delivered blocker fixes
- **Status:** pending
...
| Decision | Rationale |
|----------|-----------|
| Limit this round to the two explicit blockers from self-review | The workflow requires targeted review-fix work, not opportunistic cleanup |
| Fix the summary bug in `ChronicleService.summarize_project_timeline(...)` instead of widening the API schema | The incorrect behavior is caused by milestone rendering logic still reading the raw `closed_at` field without scope validation |
| Restore page scrolling with a local `.ptl-page` scroll context | Global `html/body/#root` overflow locking is shared by the existing app layout, so the safest fix is to give the standalone page its own `height` and `overflow-y` rules |
