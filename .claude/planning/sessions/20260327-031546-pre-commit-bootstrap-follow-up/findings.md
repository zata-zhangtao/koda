...
## 2026-03-27 Pre-commit Bootstrap Follow-up Findings
- The reported lint failure was configuration-only: `archive-tasks` still used the deprecated `commit` stage name, and `pre-commit` could not bootstrap remote GitHub hook repos because `git fetch origin --tags` against `https://github.com/pre-commit/pre-commit-hooks/` failed with `gnutls_handshake() failed`.
- Leaving either `https://github.com/pre-commit/pre-commit-hooks` or `https://github.com/astral-sh/ruff-pre-commit` in `.pre-commit-config.yaml` would keep `pre-commit` dependent on external Git clone/bootstrap behavior, so the narrowest robust fix is to convert both to `repo: local` and execute `uv run ...` commands from the project environment.
- Once hooks are localized, the original repo manifests no longer supply file-type filtering, so `check-yaml`, `check-toml`, `debug-statements`, and `ruff` need explicit `types` declarations in `.pre-commit-config.yaml` to avoid false failures on unrelated file formats.
- After adding `pre-commit-hooks` and `ruff` to `pyproject.toml`, refreshing `uv.lock`, and syncing the environment, `uv run pre-commit run --all-files` passes end-to-end without attempting any GitHub clone.
- `tasks/prd-627540a9.md` was reviewed and intentionally left unchanged because this round repairs lint/bootstrap infrastructure only and does not modify the timeline feature contract.

## 2026-03-27 Task Reference Length Guard Follow-up Findings
- `dsl/api/tasks.py:create_task_reference(...)` previously validated existence/ownership/editability but still concatenated the final `requirement_brief` text blindly, so strict databases would translate an oversized reference appendix into a late `500`.
- The smallest blocker-only fix is to validate the pending `requirement_brief` payload before any write-side effects and reuse the existing structured `TRANSFERRED` log as the overflow escape hatch.
- A compact appendix that preserves `## Referenced Requirement Context` plus `- Source Task ID: ...` is sufficient to keep `_requirement_brief_contains_reference(...)` and idempotency semantics intact even when the full source summary cannot fit.
- If there is not enough room for even that compact audit stub, the API should stop early with `422`; allowing `TaskReferenceLink` or `DevLog` creation before the failure would leave a misleading half-success state for “加入当前需求卡片”.
- Focused verification is green after the follow-up: `22 passed` across `tests/test_project_timeline_api.py`, `tests/test_project_service.py`, and `tests/test_database.py`, plus `just docs-build` and `git diff --check` on the touched files.

## 2026-03-27 Project Timeline Blocker Follow-up Findings
- `dsl/api/tasks.py:create_task_reference(...)` currently creates a new `TRANSFERRED` DevLog on every call and appends the same requirement appendix repeatedly, because there is no persisted source-target relation or any fallback dedup check.
- The PRD already promised a dedicated `task_reference_link` relation for “加入当前需求卡片”, so adding that table is a contract-restoring fix rather than feature creep.
- The narrowest backwards-compatible API behavior is: create the relation and log on first call, reuse the existing relation/log on later calls, and only append to `requirement_brief` once.
- `frontend/src/App.tsx` already excludes `ABANDONED` from editable states and includes it in archived/change filtering, but there is no user-triggered action that can ever set that status.
- A dedicated `Abandon` action should mirror `Delete`: confirmation dialog, `updateStatus(..., ABANDONED)`, a structured DevLog marker, and routing into the existing “changes” view.
- The implemented data model uses `TaskReferenceLink(run_account_id, source_task_id, target_task_id, reference_log_id, requirement_brief_appended, created_at)` with a unique `source_task_id + target_task_id` constraint, which satisfies the PRD's audit and dedup requirement without changing the existing API shape.
- Focused verification is green after the fix: `20 passed` across `tests/test_project_timeline_api.py`, `tests/test_project_service.py`, and `tests/test_database.py`, plus frontend and MkDocs builds.

## 2026-03-27 Project Category Timeline Findings
- The repository already has a dedicated `/project-timeline` page and matching `chronicle` API/service methods, but they are scoped to a single `project_id` and do not model project categories.
- `dsl/models/project.py`, `dsl/schemas/project_schema.py`, `dsl/api/projects.py`, and `frontend/src/App.tsx` currently expose project name/path/description only; there is no persistent `project_category` field anywhere in the stack.
- The current timeline page lists task-level entries with log counts and opens task detail for raw logs; this interaction model can stay intact if timeline entries are enriched with project/category metadata and the query scope is widened.
- The current “加入当前需求卡片” flow filters candidate target tasks by the page-level selected project, so category-wide browsing needs a fallback that uses the selected task's owning project to avoid offering unrelated targets.
- Existing docs already describe `Project` fields in `docs/database/schema.md` and the backend/frontend layering in `docs/guides/dsl-development.md`; both need updates because this change adds a persistent project field and changes how timeline browsing scope works.
- The current worktree already contains unrelated modifications from prior timeline/task-artifact work, so this task must be integrated without reverting or reformatting those unrelated deltas.
- The first self-review blocker is in `ChronicleService.get_project_timeline(...)`: it aggregates all historical `DevLog` rows first, then applies `start_date/end_date` against a task-level `last_activity_at`, which drops tasks that do have in-window logs when a later close/update exists outside the range.
- The same date-filter bug also poisons summary output because `summarize_project_timeline(...)` reuses timeline entries and therefore inherits full-history `bug_count` / `fix_count` values.
- The second blocker is frontend-only: `frontend/src/pages/ProjectTimelinePage.tsx` hard-codes its default lifecycle filter list and forgot `TaskLifecycleStatus.PENDING`, so pending tasks never appear in either the page list or the summary request payload.
- There is no frontend test harness in `frontend/package.json`, so the narrowest regression guard for the PENDING visibility fix is a static contract test that checks the shared frontend status constant and the page's reuse of it.
...
## 2026-03-27 Project Timeline Blocker Fix Findings
- The remaining self-review blockers are narrowly scoped to `dsl/services/chronicle_service.py` summary rendering and `frontend/src/index.css` standalone timeline page layout.
- `ChronicleService.get_project_timeline(...)` already computes `last_activity_at` from in-window events only, but `summarize_project_timeline(...)` still turns every currently `CLOSED` task into an “已完成” milestone without checking whether `closed_at` itself belongs to the requested window.
- The blocking summary repro is stable in a minimal SQLite script: a task with a `2026-03-05` in-window log and a `2026-03-10` close still renders `已完成（2026-03-10 ...）` for the `2026-03-04` to `2026-03-06` window.
- The standalone `/project-timeline` route renders `ProjectTimelinePage` directly under `#root`, while global CSS keeps `html`, `body`, and `#root` at `overflow: hidden`; without a local scroll container, any detail content past one viewport is clipped.
- There is no browser test harness in the repo, so the narrowest frontend regression guard is a static source assertion that the `.ptl-page` CSS block now declares an explicit vertical scroll container.
