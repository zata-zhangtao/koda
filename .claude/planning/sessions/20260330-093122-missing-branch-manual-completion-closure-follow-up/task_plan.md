# Task Plan: Missing Branch Manual Completion Closure Follow-up

**Goal**: Close the remaining gaps from the missing-branch manual-complete feature by making the frontend honor card-metadata branch-health updates without a hard refresh, blocking normal `/complete` for missing-branch candidates, and adding regression coverage for both paths.
**Started**: 2026-03-30

## Phases

### Phase 1: Discovery
- [x] Re-read the blocking review notes and inspect the current frontend metadata polling path
- [x] Confirm whether `/api/tasks/{id}/complete` still allows missing-branch candidates through
- [x] Identify the narrowest patch that respects existing in-flight user changes
- **Status:** complete

### Phase 2: Implementation
- [x] Update frontend branch-health resolution so the selected-task detail and completion CTA follow metadata polling state
- [x] Reject standard `/complete` when `manual_completion_candidate=true` and direct callers should use `/manual-complete`
- [x] Add regression tests for the `/complete` rejection path and verify the frontend stale-snapshot fix through build validation
- **Status:** complete

### Phase 3: Verification & Delivery
- [x] Run focused backend/frontend verification plus docs build if touched
- [x] Perform a diff review with the code-reviewer checklist
- [x] Update the task PRD with the delivered closure details if behavior changed materially
- **Status:** complete
...
| Decision | Rationale |
|----------|-----------|
| Treat task-card metadata as the authoritative source for `branch_missing` display/CTA state while the task remains active | The whole purpose of the metadata poller is to surface branch disappearance without waiting for a full task-list refresh |
| Keep the branch-health source fallback to task snapshots | Full task refreshes should still behave correctly even if metadata polling fails temporarily |
| Block standard `/complete` in the service layer rather than only in the API route | This protects direct API callers and any future code path that reuses completion preparation |

## Completion Summary
- **Status:** Complete (2026-03-30)
- **Tests:**
  - `uv run pytest tests/test_tasks_api.py -q` -> PASS (`19 passed`)
  - `cd frontend && npm run build` -> PASS
  - `just docs-build` -> PASS
  - `git diff --check -- frontend/src/App.tsx dsl/api/tasks.py dsl/services/task_service.py tests/test_tasks_api.py tasks/prd-0fd7ed62.md` -> PASS
- **PRD:** Updated `tasks/prd-0fd7ed62.md`
- **Deliverables:** `frontend/src/App.tsx`, `dsl/api/tasks.py`, `dsl/services/task_service.py`, `tests/test_tasks_api.py`, `tasks/prd-0fd7ed62.md`
- **Notes:** No standalone frontend test runner exists in this repo, so the stale-snapshot UI fix was verified through the targeted logic change plus a successful production build.

# Task Plan: Missing Branch Manual Completion Review Fix

**Goal**: Tighten the missing-branch manual-complete eligibility so never-started linked tasks do not surface as `branch_missing`, add regression coverage, and resync docs/PRD with the stricter gate.
**Started**: 2026-03-27

## Phases

### Phase 1: Discovery
- [x] Re-read the blocking self-review finding and inspect the current `manual_completion_candidate` gate
- [x] Confirm where `worktree_path` becomes available and which UI/API paths consume `branch_missing`
- [x] Decide on the narrowest safe eligibility rule that blocks backlog false positives without changing normal `/complete`
- **Status:** complete

### Phase 2: Implementation
- [x] Tighten backend manual-completion eligibility to require a task that has already entered the worktree-backed Git flow
- [x] Update user-facing/docs wording so `branch_missing` is described as a post-worktree/manual-merge exception path
- **Status:** complete

### Phase 3: Verification & Delivery
- [x] Add a regression test for linked backlog tasks not surfacing `branch_missing`
- [x] Run focused tests, docs validation, and a diff review
- [x] Update `tasks/prd-0fd7ed62.md` with the delivered gate refinement
- **Status:** complete
...
| Decision | Rationale |
|----------|-----------|
| Use persisted `worktree_path` as the new eligibility gate for `manual_completion_candidate` | It is the existing durable signal that the task actually entered Koda's git-backed flow, and it blocks the reported backlog false positive without broadening persistence scope |
| Keep branch probing itself enabled even when manual completion is not allowed | The UI can still show branch state diagnostics, but only eligible tasks should unlock the `branch_missing` completion path |

## Completion Summary
- **Status:** Complete (2026-03-27)
- **Tests:**
  - `uv run pytest tests/test_tasks_api.py tests/test_task_service.py -q` -> PASS (`27 passed`)
  - `just docs-build` -> PASS
  - `git diff --check -- dsl/services/task_service.py tests/test_tasks_api.py docs/architecture/system-design.md docs/guides/dsl-development.md docs/index.md tasks/prd-0fd7ed62.md` -> PASS
- **PRD:** Updated `tasks/prd-0fd7ed62.md`
- **Deliverables:** `dsl/services/task_service.py`, `tests/test_tasks_api.py`, `docs/architecture/system-design.md`, `docs/guides/dsl-development.md`, `docs/index.md`, `tasks/prd-0fd7ed62.md`
- **Notes:** Diff review found no additional blocking issues after the eligibility gate was tightened.

# Task Plan: Missing Branch Completion Confirmation PRD

**Goal**: Produce a context-aware PRD for the case where a requirement branch was manually merged and deleted outside Koda, add an interactive prototype that demonstrates the expected confirmation flow, update MkDocs navigation, and verify the documentation build.
**Started**: 2026-03-26

## Phases

### Phase 1: Discovery
- [x] Read the `/prd` skill contract and inspect the existing task/worktree/task-card implementation
- [x] Identify how the current app derives completion state and where branch existence is not checked
- [x] Capture implementation constraints in planning notes
- **Status:** complete

### Phase 2: PRD & Prototype Authoring
- [x] Write the PRD to `tasks/prd-0fd7ed62.md` with required metadata, visuals, and clarifying questions
- [x] Create a prototype page under `docs/prototypes/` showing the missing-branch confirmation flow
- [x] Update `mkdocs.yml` navigation for the new prototype
- **Status:** complete

### Phase 3: Verification & Delivery
- [x] Run docs validation
- [x] Review the diff for quality and consistency
- [x] Record outcomes in planning files and deliver the generated file path
- **Status:** complete
...
| Decision | Rationale |
|----------|-----------|
| Treat the feature as a branch-health detection plus user-confirmed completion flow | The user explicitly wants a click-to-check/complete path when the branch is already gone, not silent auto-closing |
| Use a prototype page in `docs/prototypes/missing-branch-complete-demo.html` | The `/prd` skill requires a behavior demo when the requirement includes state transitions and button flows |
| Keep the PRD output path fixed to `tasks/prd-0fd7ed62.md` | The user provided an explicit output contract that overrides the skill's default timestamp naming |

## Completion Summary
- **Status:** Complete (2026-03-26)
- **PRD:** Created `tasks/prd-0fd7ed62.md`
- **Deliverables:** `tasks/prd-0fd7ed62.md`, `docs/prototypes/missing-branch-complete-demo.html`, `mkdocs.yml`
- **Notes:** `just docs-build` passed, and a manual diff review found no blocking issues in the new prototype/nav changes.

---

# Task Plan: Implement Missing Branch Manual Completion Flow

**Goal**: Implement the missing-branch detection and manual completion flow described in `tasks/prd-0fd7ed62.md`, including backend branch-health projection, a dedicated manual-complete endpoint, frontend inspection/confirmation UX, synchronized docs, and regression coverage.
**Started**: 2026-03-27

## Phases

### Phase 1: Discovery
- [x] Re-read `tasks/prd-0fd7ed62.md` and confirm the exact acceptance criteria and non-goals
- [x] Inspect current task API, task service, task schemas, frontend completion UI, and existing tests
- [x] Identify the narrowest repo-root / branch-probe implementation that does not disturb the normal `/complete` path
- **Status:** complete

### Phase 2: Backend Implementation
- [x] Add a branch-health read model derived from canonical task branch naming
- [x] Add a dedicated manual-complete API path with validation, audit log creation, and `done/CLOSED` convergence
- [x] Extend task/card metadata serialization for the new UI state without changing normal completion behavior
- **Status:** complete

### Phase 3: Frontend Implementation
- [x] Extend task types and API client for branch-health and manual-complete
- [x] Add missing-branch detail messaging, inspection gating, and manual confirmation flow
- [x] Preserve the existing `Complete` UX when the task branch still exists
- **Status:** complete

### Phase 4: Verification & Sync
- [x] Add regression tests for branch exists, branch missing, validation failure, and successful manual completion
- [x] Update docs/PRD with actual deliverables and validation evidence
- [x] Run focused tests plus docs/frontend validation, then perform a code review pass
- **Status:** complete
...
| Decision | Rationale |
|----------|-----------|
| Keep branch health as a derived response model instead of a persisted DB field | The PRD explicitly forbids schema expansion for this feature |
| Reuse `TaskService` for the final manual-complete state convergence | That service already owns `workflow_stage` and `lifecycle_status` transitions, so it is the safest place for a dedicated closure helper |
| Reject `/manual-complete` while automation is still running | Frontend already disables that CTA, and the backend must not allow a direct API caller to close a task mid-automation |

## Completion Summary
- **Status:** Complete (2026-03-27)
- **Tests:**
  - `uv run python -m py_compile dsl/api/tasks.py dsl/services/task_service.py dsl/services/git_worktree_service.py dsl/schemas/task_schema.py tests/test_tasks_api.py` -> PASS
  - `uv run pytest tests/test_tasks_api.py tests/test_task_service.py tests/test_git_worktree_service.py -q` -> PASS (`31 passed`)
  - `cd frontend && npm run build` -> PASS
  - `just docs-build` -> PASS
  - `git diff --check` -> PASS
- **PRD:** Updated `tasks/prd-0fd7ed62.md`
- **Deliverables:** `dsl/api/tasks.py`, `dsl/services/task_service.py`, `dsl/services/git_worktree_service.py`, `dsl/schemas/task_schema.py`, `frontend/src/App.tsx`, `frontend/src/api/client.ts`, `frontend/src/types/index.ts`, `frontend/src/index.css`, `tests/test_tasks_api.py`, `docs/guides/dsl-development.md`, `docs/architecture/system-design.md`, `docs/index.md`, `tasks/prd-0fd7ed62.md`
- **Notes:** Normal `/api/tasks/{id}/complete` remains unchanged; the new manual path only activates when `branch_health.manual_completion_candidate=true`.
