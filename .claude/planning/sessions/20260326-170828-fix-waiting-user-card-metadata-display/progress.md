...
- Planned the blocker fix around two targeted frontend changes: broaden the active `/tasks` polling trigger to any running task plus transition-based metadata refresh, and replace metadata errors with a task-derived fallback map.
- Implemented the blocker-only follow-up in `frontend/src/App.tsx`: metadata failure fallback now always rebuilds from the newest available task snapshot, and `resolveTaskCardMetadata(...)` rejects cached metadata that no longer matches the latest task state while preserving the newest AI activity timestamp.
