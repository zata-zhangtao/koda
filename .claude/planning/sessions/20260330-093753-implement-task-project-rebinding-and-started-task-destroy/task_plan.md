...
## 2026-03-30 Review Fix Follow-Up

**Goal**: Resolve the blocking review findings in the existing task project rebinding / started-task destroy implementation so started tasks cannot bypass destroy constraints and destroy only succeeds after confirmed cleanup.
**Started**: 2026-03-30

### Phase 1: Review Gap Confirmation
- [x] Re-read the active PRD, implementation diff, and review summary
- [x] Confirm whether backlog-only project rebinding is already wired end-to-end
- [x] Isolate the exact deletion-bypass and cleanup-verification gaps in backend code/tests
- **Status:** complete

### Phase 2: Backend Tightening
- [x] Block direct `PUT /api/tasks/{task_id}/status` deletion for started tasks and require the dedicated destroy flow
- [x] Require destroy cleanup to confirm both worktree removal and branch deletion before persisting `DELETED`
- [x] Keep backlog delete behavior intact for non-started tasks
- **Status:** complete

### Phase 3: Verification And Review
- [x] Add regression coverage for the closed bypasses
- [x] Run focused backend tests and targeted diff review
- [x] Sync planning / PRD delivery notes with the actual fix set
- **Status:** complete

## Current Phase
Review-fix follow-up complete ✅
...
All original implementation phases complete ✅
...
- **Review Fixes (2026-03-30):**
  - Closed the legacy `/status -> DELETED` bypass for started tasks; the route now returns 422 unless the task is still a backlog lightweight-delete case.
  - Tightened destroy cleanup completion so both the worktree directory and task branch must actually disappear before the task is archived.
  - Added regression coverage in `tests/test_task_service.py`, `tests/test_tasks_api.py`, and `tests/test_git_worktree_service.py`, then reran focused pytest plus `just docs-build`.
