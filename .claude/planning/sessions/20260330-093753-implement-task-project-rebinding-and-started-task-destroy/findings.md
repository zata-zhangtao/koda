...
## 2026-03-30 Review Fix Findings For Task Project Rebinding And Started-Task Destroy
- The backlog project rebinding flow is already present end-to-end in the current worktree: `frontend/src/App.tsx` now seeds/submits `editRequirementProjectId`, `frontend/src/api/client.ts` can send `project_id`, and `dsl/api/tasks.py` / `dsl/services/task_service.py` already persist backlog-only rebinding with an audit log.
- The original user-visible complaint ("选错项目就不能改了") is therefore already addressed by the current implementation; the remaining blockers are backend integrity gaps called out by review.
- `dsl/api/tasks.py:update_task_status(...)` still forwards every lifecycle change directly into `TaskService.update_task_status(...)` without rejecting `DELETED` for started tasks, so callers can bypass the dedicated destroy endpoint and skip required `destroy_reason` / cleanup semantics.
- `dsl/services/task_service.py:update_task_status(...)` currently stamps `destroyed_at` for any direct `DELETED` transition but does not enforce "backlog-only lightweight delete" or clear stale runtime fields, which is the service-layer half of the deletion bypass.
- `dsl/api/tasks.py:destroy_task(...)` currently treats `cleanup_succeeded=True` as enough to archive the task even though `WorktreeDestroyResult` separately reports `worktree_removed` and `branch_deleted`; this is precisely the review-reported false-positive cleanup path.
- The safest fix is defense in depth: tighten `TaskService.update_task_status(...)` / `update_task_status(...)` so only non-started tasks can use the old delete path, and tighten `destroy_task(...)` plus `GitWorktreeService.destroy_task_worktree(...)` so destroy refuses partial cleanup even when a repo-local cleanup script exits `0`.

## 2026-03-26 Implementation Findings For Task Project Rebinding And Started-Task Destroy
- `tasks/prd-df3b22d8.md` is already present and should be treated as the implementation contract to sync before delivery.
- The planning session was already initialized under `.claude/planning/current/`, so this execution can reuse the active files instead of forcing a fresh session.
- `git status --short` shows only documentation/PRD work (`mkdocs.yml`, `docs/prototypes/task-project-rebinding-and-destroy-demo.html`, `tasks/prd-df3b22d8.md`); the actual backend/frontend implementation for this card has not landed yet.
- Repo search confirms the main backend touchpoints are `dsl/models/task.py`, `dsl/schemas/task_schema.py`, `dsl/services/task_service.py`, `dsl/api/tasks.py`, `dsl/services/git_worktree_service.py`, and `utils/database.py`.
- Repo search confirms the main frontend touchpoints are `frontend/src/App.tsx`, `frontend/src/api/client.ts`, and `frontend/src/types/index.ts`.
- Existing docs already describe `TaskResponse.is_codex_task_running` and `worktree_path` semantics, so the new destroy/rebinding behavior must stay aligned with those contracts rather than inventing a parallel runtime state model.
- `dsl/models/task.py` currently has no persisted `destroy_reason` or `destroyed_at` fields, and `dsl/schemas/task_schema.py` likewise cannot expose them yet.
- `TaskService.update_task_title(...)` only updates title and optional `requirement_brief`; there is no service-layer validation for backlog-only project rebinding.
- `dsl/api/tasks.py` currently exposes `PATCH /api/tasks/{task_id}` and `PUT /api/tasks/{task_id}/status`, but no dedicated destroy endpoint. The existing `POST /api/tasks/{task_id}/cancel` already knows how to stop Codex runtime state and is the likely destroy-flow integration point.
- `utils/database.py` uses an `_INCREMENTAL_SCHEMA_PATCHES` tuple for SQLite-safe additive migrations, which is the correct place to backfill new `tasks.destroy_reason` and `tasks.destroyed_at` columns.
- `frontend/src/api/client.ts` only lets `taskApi.update(...)` send `task_title` and `requirement_brief`, and the `Task` type has no destroy-related fields.
- `frontend/src/App.tsx` uses `handleDeleteRequirement(...)` with `window.confirm(...)` and `taskApi.updateStatus(..., DELETED)`, so started-task destroy must be introduced as a separate UI path rather than a small tweak to the current confirm dialog.
- The requirement editor in `frontend/src/App.tsx` seeds title/summary state only; it currently does not preload or submit any editable `project_id`, even though the create flow already has project selection state elsewhere in the file.
- `frontend/src/App.tsx:isProjectSelectable(...)` already encodes the project dropdown eligibility rule used by the create form, so the edit form can reuse the same filter without inventing a second rule set.
- `dsl/api/tasks.py:/cancel` already performs the exact runtime-side effects needed before destroy (`cancel_codex_task(...)` + `clear_task_background_activity(...)`), but it currently also forces the task back to `changes_requested`, so destroy should not call the endpoint directly.
- `dsl/services/codex_runner.py` contains completion-time cleanup logic, but that path assumes the task branch was merged and uses cleanup semantics that are not ideal for abandoned destroy flows. Started-task destroy likely needs a dedicated cleanup helper that can force-delete the task branch after removing the worktree.
- `scripts/git_worktree_merge.sh --delete` deletes the local branch with `git branch -d`, which may fail for unmerged abandoned task branches. Relying on that script alone would make destroy brittle for the exact stale-worktree scenario the feature is meant to clean up.
- `dsl/services/codex_runner.py:_execute_git_completion_flow(...)` confirms the fallback cleanup path also uses `git branch -d`, so destroy cleanup cannot safely reuse completion cleanup as-is.
- `frontend/src/App.tsx` already has modal/drawer patterns and enough local state (`selectedTaskSnapshot`, `selectedTaskStage`, `projectList`) to add backlog-only project rebinding plus a dedicated destroy-reason modal without changing dashboard loading shape.
- `docs/database/schema.md` already documents `Task` as the source of truth for worktree and runtime-related fields, so the new destroy metadata should be documented alongside those core task fields.
- Backend implementation now exists in `dsl/models/task.py`, `dsl/schemas/task_schema.py`, `dsl/services/task_service.py`, `dsl/api/tasks.py`, `dsl/services/git_worktree_service.py`, and `utils/database.py`; focused tests passed for backlog project rebinding, started-task destroy, audit logging, and cleanup orchestration.
- Post-change review found one contract gap: the first destroy audit log version omitted task title and final requirement summary even though the PRD required both. The helper was updated and the regression test now asserts those fields are present.

## 2026-03-26 PRD Findings For Task Project Rebinding And Destroy
- `frontend/src/App.tsx` already exposes a project selector only in the create panel; the edit panel (`Requirement Revision`) has no project field.
- `frontend/src/api/client.ts` and `dsl/schemas/task_schema.py` confirm the update contract currently supports only `task_title` and `requirement_brief`, so project rebinding is impossible after creation.
- `dsl/services/task_service.py:start_task(...)` creates or reuses worktree context from the current `project_id`, which means project changes must be locked once the task leaves `backlog`.
- `frontend/src/App.tsx:handleDeleteRequirement(...)` currently uses `window.confirm(...)` and then sets `lifecycle_status=DELETED`; it does not ask for or persist a destroy reason.
- `dsl/api/tasks.py:/cancel` and `dsl/services/git_worktree_service.py` provide the natural integration points for a future started-task destroy flow that first stops automation and then cleans up worktree state.
- The attached screenshot `/home/atahang/codes/koda/data/media/original/816b5b30-9d6e-457c-9d1f-9aff9df0f959.png` exists and visibly shows the detail header plus the `Requirement Revision` panel without a project selector or destroy-reason input.
- The `/prd` skill requires a real interactive prototype for this feature because the behavior depends on state transitions and action gating, not just static copy.
