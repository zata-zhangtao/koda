...
## Session: 2026-03-30 Review Fixes For Task Project Rebinding And Started-Task Destroy
...
### Actions Taken
- Re-read the active planning files, PRD, and current implementation diff for the project rebinding / started-task destroy work.
- Confirmed the user-facing project rebinding flow is already present in the current frontend/backend code, so the remaining task is to close backend review blockers rather than re-implement the original feature.
- Located the first blocker in `dsl/api/tasks.py:update_task_status(...)` and `dsl/services/task_service.py:update_task_status(...)`: direct `DELETED` transitions for started tasks are still possible and bypass the dedicated destroy path.
- Located the second blocker in `dsl/api/tasks.py:destroy_task(...)`: the endpoint checks only `cleanup_succeeded`, not the explicit `worktree_removed` / `branch_deleted` flags.
- Confirmed `dsl/services/git_worktree_service.py:destroy_task_worktree(...)` currently marks script-driven cleanup as successful on exit code alone, so the fix should tighten both the service result and the API gate.
- Updated `dsl/services/task_service.py` so started tasks can no longer reach `DELETED` through the legacy status endpoint, while backlog lightweight delete remains available.
- Updated `dsl/api/tasks.py` so the status route translates the new validation into `422`, and the destroy route now requires `cleanup_succeeded`, `worktree_removed`, and `branch_deleted` together before archiving.
- Updated `dsl/services/git_worktree_service.py` so a repo-local cleanup script that exits `0` but leaves artifacts now falls back to force cleanup instead of reporting false success.
- Added focused regressions in `tests/test_task_service.py`, `tests/test_tasks_api.py`, and `tests/test_git_worktree_service.py` for the closed bypass and partial-cleanup cases.
- Synced `tasks/prd-df3b22d8.md` with the review-fix deliverables and latest verification evidence.

### Test Results
| Test | Expected | Actual | Status |
|------|----------|--------|--------|
| Discovery scan of current diff/routes/tests | Isolate the two review blockers before editing | Completed | passed |
| `python -m py_compile dsl/services/task_service.py dsl/api/tasks.py dsl/services/git_worktree_service.py tests/test_task_service.py tests/test_tasks_api.py tests/test_git_worktree_service.py` | Edited backend code and focused tests remain syntactically valid | Passed | passed |
| `uv run pytest tests/test_task_service.py tests/test_tasks_api.py tests/test_git_worktree_service.py -q` | Review-fix regressions and existing task/worktree coverage should stay green | `38 passed, 4 warnings` | passed |
| `git diff --check -- dsl/services/task_service.py dsl/api/tasks.py dsl/services/git_worktree_service.py tests/test_task_service.py tests/test_tasks_api.py tests/test_git_worktree_service.py tasks/prd-df3b22d8.md` | No whitespace or patch-format issues in the follow-up fix set | Passed | passed |
| `just docs-build` | Documentation should still build after PRD sync | Passed | passed |
...
## Session: 2026-03-26 Implementation For Task Project Rebinding And Started-Task Destroy
...
### Actions Taken
- Read the `planning-with-files` and `code-reviewer` skill instructions because this task spans multiple backend/frontend/doc surfaces and will need a post-change review pass.
- Reused the active planning workspace under `.claude/planning/current/` instead of forcing a new session.
- Checked `git status --short` and confirmed the only current worktree changes are the earlier PRD/prototype/doc files, not the feature implementation itself.
- Read the approved implementation contract at `tasks/prd-df3b22d8.md`.
- Ran a repo-wide search to locate the current task update/delete/start flow, task schema/model references, frontend detail editing code, and existing docs that mention `workflow_stage`, `worktree_path`, and `is_codex_task_running`.
- Identified the primary backend files (`dsl/models/task.py`, `dsl/schemas/task_schema.py`, `dsl/services/task_service.py`, `dsl/api/tasks.py`, `dsl/services/git_worktree_service.py`, `utils/database.py`) and frontend files (`frontend/src/App.tsx`, `frontend/src/api/client.ts`, `frontend/src/types/index.ts`) that must be inspected next.
- Read the task model/schema/service/API files and confirmed there is no persisted destroy metadata, no backlog-only `project_id` update logic, and no dedicated destroy endpoint.
- Read the relevant `frontend/src/App.tsx` sections and confirmed the current delete action is a simple confirm dialog plus `lifecycle_status=DELETED`, while the requirement editor only edits title/summary and does not surface project rebinding.
- Confirmed the database migration mechanism already exists as additive SQL patches, so this feature can remain compatible with existing SQLite databases without introducing a separate migration framework.
- Read the project selector helper in the create form and confirmed the edit flow can directly reuse `isProjectSelectable(...)`.
- Inspected the existing worktree cleanup path in `dsl/services/codex_runner.py` and the repo-local `scripts/git_worktree_merge.sh`; the merge-time cleanup script uses non-force branch deletion, so destroy needs its own cleanup helper for unmerged task branches.
- Read the existing modal/drawer and edit-state wiring in `frontend/src/App.tsx` / `frontend/src/index.css`; the UI can absorb a small destroy-reason modal and project lock messaging without changing data-fetch topology.
- Implemented backend support for `destroy_reason` / `destroyed_at`, backlog-only `project_id` rebinding, explicit started-task destroy validation, audit log generation, and force-cleanup worktree fallback logic.
- Added focused service/API regressions for project rebinding and started-task destroy.
- Implemented the frontend project rebinding selector, started-task destroy modal, lock messaging, and detail-view surfacing for `destroy_reason` / `destroyed_at`.
- Synchronized `docs/database/schema.md`, `docs/dev/evaluation.md`, `docs/api/references.md`, and `tasks/prd-df3b22d8.md` with the shipped behavior and verification evidence.
- Performed a post-implementation code review pass, found one real gap in the destroy audit log (missing task title / final requirement summary), fixed it, and reran verification.

### Test Results
| Test | Expected | Actual | Status |
|------|----------|--------|--------|
| Discovery scan only | Locate code/docs entry points before editing | Completed | passed |
| `uv run python -m py_compile dsl/models/task.py dsl/schemas/task_schema.py dsl/services/task_service.py dsl/services/git_worktree_service.py dsl/api/tasks.py utils/database.py` | Backend files should remain syntactically valid after the contract changes | Passed | passed |
| `uv run pytest tests/test_task_service.py tests/test_tasks_api.py -q` | Rebinding + destroy regressions should pass | `28 passed, 2 warnings` | passed |
| `cd frontend && npm run build` | Frontend should compile after adding project rebinding and destroy UX | Build succeeded | passed |
| `just docs-build` | Docs should stay valid after API/model/reference updates | Build succeeded | passed |
| `git diff --check -- dsl/models/task.py dsl/schemas/task_schema.py dsl/services/task_service.py dsl/services/git_worktree_service.py dsl/api/tasks.py frontend/src/App.tsx frontend/src/api/client.ts frontend/src/types/index.ts frontend/src/index.css docs/database/schema.md docs/dev/evaluation.md docs/api/references.md tests/test_task_service.py tests/test_tasks_api.py mkdocs.yml docs/prototypes/task-project-rebinding-and-destroy-demo.html tasks/prd-df3b22d8.md` | No whitespace or patch-format regressions in delivered files | Passed | passed |
...
## Session: 2026-03-26 PRD For Task Project Rebinding And Started-Task Destroy
...
### Actions Taken
- Read the `/prd` skill and then scanned the current frontend/backend task flow around creation, editing, start, delete, cancel, and worktree lifecycle.
- Inspected the attached screenshot directly and extracted only confirmable UI facts: detail header actions are visible, the edit panel lacks a project selector, and no destroy-reason UI exists.
- Confirmed the current update contract cannot modify `project_id`, while `start_task(...)` binds runtime worktree behavior to the current project selection.
- Confirmed the current delete path is a light soft-delete with a generic confirm dialog and no structured destroy reason.
- Added `docs/prototypes/task-project-rebinding-and-destroy-demo.html` as an interactive behavior demo.
- Updated `mkdocs.yml` to expose the prototype page under a new `原型` navigation section.
- Authored `tasks/prd-df3b22d8.md` with required metadata, clarifying questions, change matrix, Mermaid flow, ER diagram, low-fidelity prototype, and prototype change log.

### Test Results
| Test | Expected | Actual | Status |
|------|----------|--------|--------|
| `just docs-build` | MkDocs should still build after adding the prototype and nav entry | Build succeeded; upstream Material 2.0 warning banner only | passed |
