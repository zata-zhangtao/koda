# Task Plan: Archived Feedback Review Fixes

**Goal**: Resolve the two blocker regressions from archived-task feedback enforcement: keep archive-transition audit logs working, and clean up stored media when archived-task uploads are rejected.
**Started**: 2026-03-26

## Phases

### Phase 1: Scope Confirmation
- [x] Re-read the blocking self-review findings and confirm the fix stays limited to archive-transition logs and archived media cleanup
- [x] Inspect the affected frontend handlers, task APIs, log service, media API, and existing regression coverage
- **Status:** complete

### Phase 2: Implementation
- [x] Move the three archive-transition audit logs behind backend-internal writes so archived formal-feedback rejection no longer breaks accept/complete/delete
- [x] Remove the frontend's post-archive `logApi.create(...)` calls so the UI no longer reports false failures after a successful archive transition
- [x] Delete stored image/attachment files when archived-task log creation is rejected
- **Status:** complete

### Phase 3: Verification & Sync
- [x] Add regression coverage for archive audit logs and archived media cleanup
- [x] Re-run focused pytest coverage, frontend build, and docs build
- [x] Update docs and sync `tasks/prd-514e2c11.md` with the delivered blocker fix
- **Status:** complete
...
| Decision | Rationale |
|----------|-----------|
| Keep the archived-task formal-feedback guard in `LogService.create_log()` and add a backend-only internal log path | The blocker was about system audit logs being misclassified as user feedback; the rejection rule itself was correct |
| Put archive-transition audit logging in task API routes and remove the frontend follow-up writes | This is the narrowest fix that restores the three broken UI flows without reopening archived formal feedback |
| Clean up saved media files inside the API rejection path instead of broadening log permissions for attachments | The product rule is still “archived tasks reject new formal feedback”; rejected uploads should therefore leave no persisted media |

## Completion Summary
- **Status:** Complete (2026-03-26)
- **Tests:**
  - `uv run pytest tests/test_tasks_api.py tests/test_logs_api.py tests/test_media_api.py -q` -> PASS (`23 passed`)
  - `uv run python -m py_compile dsl/api/tasks.py dsl/api/media.py dsl/services/log_service.py dsl/services/media_service.py` -> PASS
  - `cd frontend && npm run build` -> PASS
  - `just docs-build` -> PASS
  - `git diff --check` -> PASS
- **PRD:** Updated `tasks/prd-514e2c11.md`
- **Deliverables:**
  - `dsl/services/log_service.py`, `dsl/api/tasks.py`
  - `dsl/api/media.py`, `dsl/services/media_service.py`
  - `frontend/src/App.tsx`
  - `tests/test_tasks_api.py`, `tests/test_media_api.py`
  - `docs/architecture/system-design.md`, `docs/dev/evaluation.md`

# Task Plan: Independent Task Q&A Review Fixes

**Goal**: Resolve the three blocking review findings in the task-scoped sidecar Q&A feature: keep archived Q&A history visible after task completion, reject blank questions defensively, and prevent pending replies from locking a task forever.
**Started**: 2026-03-26

## Phases

### Phase 1: Reproduction & Scope
- [x] Re-read the review findings and inspect the affected frontend/backend code paths
- [x] Confirm the fix stays limited to archived visibility, blank-input validation, and pending recovery
- **Status:** complete

### Phase 2: Implementation
- [x] Separate sidecar visibility from submit permissions so `CLOSED` tasks keep history and feedback-draft conversion in read-only mode
- [x] Reject blank question payloads in both schema and service normalization paths
- [x] Add model timeout controls plus automatic stale-pending release to failed state
- **Status:** complete

### Phase 3: Verification & Sync
- [x] Add/update regression tests for blank input, stale pending recovery, and timeout control wiring
- [x] Re-run sidecar pytest coverage, frontend build, and docs build
- [x] Update docs and sync `tasks/prd-514e2c11.md` with the delivered review-fix behavior
- **Status:** complete
...
| Decision | Rationale |
|----------|-----------|
| Keep archived sidecar history visible but make the composer read-only on `CLOSED` tasks | The blocker only requires historical visibility and feedback-draft conversion after completion; reopening the execution lane would expand scope beyond review-fix |
| Recover stuck pending replies automatically during list/create flows instead of adding a new manual endpoint | This is the narrowest fix that removes the permanent lock without expanding the API surface during review-fix |
| Use explicit timeout/retry controls on the sidecar chat-model client | Prevents a single provider hang from leaving a reply pending indefinitely |

## Completion Summary
- **Status:** Complete (2026-03-26)
- **Tests:**
  - `uv run pytest tests/test_task_qa_api.py tests/test_task_qa_config.py -q` -> PASS (`17 passed`)
  - `cd frontend && npm run build` -> PASS
  - `just docs-build` -> PASS
- **PRD:** Updated `tasks/prd-514e2c11.md`
- **Deliverables:**
  - `dsl/schemas/task_qa_schema.py`, `dsl/services/task_qa_service.py`
  - `frontend/src/App.tsx`
  - `tests/test_task_qa_api.py`
  - `docs/architecture/system-design.md`, `docs/dev/evaluation.md`

# Task Plan: Independent Task Q&A Sidecar Implementation

**Goal**: Implement a task-scoped independent Q&A capability that stays available during `prd_waiting_confirmation` and `implementation_in_progress`, answers user questions without mutating the main automation workflow, keeps its data out of `DevLog`, and gives users an explicit path from “just asking” to “formal feedback”.
**Started**: 2026-03-26

## Phases

### Phase 1: Discovery & Contract
- [x] Read PRD, `planning-with-files`, and `code-reviewer` skill instructions
- [x] Inspect the existing feedback composer, task workflow routing, and prompt-context assembly
- [x] Confirm that sidecar Q&A needs dedicated persistence plus a model-loader-backed answer path
- **Status:** complete

### Phase 2: Backend Implementation
- [x] Add `task_qa_messages` ORM model, supporting enums, schemas, and database initialization/index coverage
- [x] Implement `TaskQaService` for question persistence, task-context assembly, graceful missing-PRD fallback, background answer generation, and feedback-draft conversion
- [x] Expose `/api/tasks/{task_id}/qa/messages` list/create routes plus feedback-draft conversion without changing `workflow_stage` or `is_codex_task_running`
- **Status:** complete

### Phase 3: Frontend & Docs
- [x] Split the task-detail composer into explicit “反馈给执行链路” vs “问 AI” channels
- [x] Add task-sidecar Q&A polling, scope selection, pending/completed/failed rendering, and latest-answer-to-feedback-draft conversion
- [x] Sync API reference, system design, AI-asset boundary, configuration, development guide, and manual evaluation docs
- **Status:** complete

### Phase 4: Verification & Review
- [x] Run targeted backend regressions for task Q&A plus existing task/database APIs
- [x] Run full Python test suite
- [x] Run frontend production build and docs build
- [x] Perform a post-change code review pass and sync the task PRD
- **Status:** complete
...
| Decision | Rationale |
|----------|-----------|
| Model the feature as a task-scoped sidecar Q&A flow, not an extension of `feedback` semantics | The current feedback composer writes `DevLog` entries and can trigger PRD regeneration / resume behavior, which conflicts with the requirement that Q&A must not affect coding flow by default |
| Persist sidecar Q&A in a dedicated `task_qa_messages` table | `dsl/api/tasks.py` serializes `DevLog` into PRD/execution prompt context, so casual Q&A stored there would pollute main automation inputs |
| Make `TASK_QA_BACKEND=chat_model` the only supported first-version backend | FR-9 explicitly prioritizes `ai_agent/utils/model_loader.py`; keeping Codex as the default would blur the boundary with the main execution chain |
| Convert sidecar conclusions into feedback drafts explicitly instead of auto-writing `DevLog` | This preserves the “just asking” contract until the user confirms they want the conclusion operationalized |
| Use `tasks/prd-514e2c11.md` as the authoritative PRD output path | The user explicitly requested this fixed path for the deliverable |
...
- **PRD:** Updated `tasks/prd-514e2c11.md`
- **Deliverables:**
  - `dsl/models/task_qa_message.py`, `dsl/schemas/task_qa_schema.py`, `dsl/services/task_qa_service.py`, `dsl/api/task_qa.py`
  - `frontend/src/App.tsx`, `frontend/src/api/client.ts`, `frontend/src/types/index.ts`, `frontend/src/index.css`
  - `docs/api/references.md`, `docs/architecture/system-design.md`, `docs/core/ai-assets.md`, `docs/dev/evaluation.md`, `docs/guides/configuration.md`, `docs/guides/dsl-development.md`
  - `tests/test_task_qa_api.py`, `tests/test_task_qa_config.py`
...
## Reopened Notes (2026-03-26)
- Self-review found two blocking gaps after the first sidecar Q&A review-fix round:
  1. Archived-task “formal feedback disabled” existed only in the frontend; backend log/media write paths still accepted the write.
  2. The “single pending assistant reply per task” rule still relied on a non-atomic read-before-write check.
- Resolution:
  - Added backend task-state enforcement in `dsl/services/log_service.py` so `CLOSED` / `DELETED` tasks reject formal feedback writes regardless of caller path.
  - Added SQLite bootstrap repair plus a unique partial index for pending assistant replies, and updated `dsl/services/task_qa_service.py` to keep stale-pending cleanup inside the current transaction and translate uniqueness races into the existing pending-conflict error.
  - Added regression coverage in `tests/test_task_qa_api.py`, `tests/test_logs_api.py`, and `tests/test_database.py`.
  - Re-ran focused sidecar/log/database pytest coverage, MkDocs build, and `git diff --check` successfully.
