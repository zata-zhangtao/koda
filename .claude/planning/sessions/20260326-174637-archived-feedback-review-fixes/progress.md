...
## Session: 2026-03-26 Archived Feedback Review Fixes
...
### Actions Taken
- Re-read the blocker findings and confirmed the fix scope was limited to archive-transition logs plus archived media cleanup.
- Added a backend-only internal log path in `LogService` so internal archive audit events can still target `CLOSED/DELETED` tasks while public formal feedback remains rejected.
- Moved the three broken archive-transition audit logs to the backend task routes and removed the frontend's follow-up `logApi.create(...)` calls after `DONE/CLOSED/DELETED`.
- Added orphan-media cleanup to image and attachment upload rejection paths so archived-task uploads no longer leave files on disk.
- Extended `tests/test_tasks_api.py` and `tests/test_media_api.py` with regressions for the three archive flows and the two media cleanup paths.
- Updated `docs/architecture/system-design.md`, `docs/dev/evaluation.md`, and `tasks/prd-514e2c11.md` to match the delivered review fix.

### Test Results
| Test | Expected | Actual | Status |
|------|----------|--------|--------|
| `uv run pytest tests/test_tasks_api.py tests/test_logs_api.py tests/test_media_api.py -q` | Archive-transition and media-cleanup regressions pass | `23 passed` | passed |
| `uv run python -m py_compile dsl/api/tasks.py dsl/api/media.py dsl/services/log_service.py dsl/services/media_service.py` | Updated backend modules compile cleanly | Succeeded | passed |
| `cd frontend && npm run build` | Frontend compiles after removing post-archive log writes | Build succeeded | passed |
| `just docs-build` | Documentation remains valid after blocker-fix updates | Build succeeded | passed |
| `git diff --check` | No whitespace or merge-marker issues remain in the patch | Succeeded | passed |
...
## Session: 2026-03-26 Independent Task Q&A Review Fixes
...
### Actions Taken
- Re-read the blocking review notes and mapped them to `frontend/src/App.tsx`, `dsl/schemas/task_qa_schema.py`, and `dsl/services/task_qa_service.py`.
- Split sidecar visibility from submit permissions so `CLOSED` tasks still poll and render Q&A history plus feedback-draft conversion, while feedback and new questions become read-only.
- Added whitespace-only question rejection in both the Pydantic schema and the service normalization path.
- Added explicit chat-model timeout/retry controls and automatic stale-pending release to `FAILED` during list/create flows.
- Extended `tests/test_task_qa_api.py` with regressions for blank questions, stale pending recovery, and timeout control wiring.
- Updated `docs/architecture/system-design.md`, `docs/dev/evaluation.md`, and `tasks/prd-514e2c11.md` to reflect the archived-history and pending-recovery behavior.

### Test Results
| Test | Expected | Actual | Status |
|------|----------|--------|--------|
| `uv run pytest tests/test_task_qa_api.py tests/test_task_qa_config.py -q` | Sidecar review-fix regressions pass | `17 passed` | passed |
| `cd frontend && npm run build` | Frontend compiles after archived-history/read-only composer changes | Build succeeded | passed |
| `just docs-build` | Docs remain valid after sidecar review-fix updates | Build succeeded | passed |

### Errors
| Error | Resolution |
|-------|------------|
| Initial stale-pending list regression assumed the expired assistant message would sort last | Changed the assertion to locate the assistant message by ID before checking its final status |

## Session: 2026-03-26 Independent Task Q&A Sidecar Implementation
...
### Actions Taken
- Re-read the generated PRD and current staged task-QA config/doc changes to decide whether to continue that direction or realign with the requirement card.
- Added new task-sidecar Q&A enums, `TaskQaMessage` ORM model, task/run-account relationships, and database indexes so Q&A no longer shares persistence with `DevLog`.
- Added `dsl/schemas/task_qa_schema.py` plus `dsl/api/task_qa.py` for list/create/feedback-draft endpoints under `/api/tasks/{task_id}/qa/messages`.
- Implemented `dsl/services/task_qa_service.py` to enforce one pending assistant reply per task, assemble task context from title/summary/stage/recent logs/PRD, degrade gracefully when the PRD file is missing, and update only the targeted message on failure.
- Aligned task-Q&A configuration with FR-9 by switching the supported default backend to `TASK_QA_BACKEND=chat_model` and reusing `ai_agent/utils/model_loader.py`.
- Updated `frontend/src/App.tsx` to split the bottom composer into “反馈给执行链路” and “问 AI”, poll sidecar messages, show pending/failed/completed states, and convert the latest answer into a feedback draft without auto-submitting it.
- Updated `frontend/src/api/client.ts`, `frontend/src/types/index.ts`, and `frontend/src/index.css` to support the new API contracts and UI states.
- Synchronized architecture/API/config/development/evaluation docs plus the task PRD with the delivered behavior and the final backend decision.
- Ran a post-change review pass over the current diff and found no blocker-level issues beyond the already-resolved backend-default mismatch.

### Test Results
| Test | Expected | Actual | Status |
|------|----------|--------|--------|
| `uv run pytest tests/test_task_qa_api.py tests/test_task_qa_config.py -q` | New sidecar Q&A configuration and service/API regressions pass | `12 passed` | passed |
| `uv run pytest tests/test_task_qa_api.py tests/test_task_qa_config.py tests/test_tasks_api.py tests/test_logs_api.py tests/test_database.py -q` | New task-Q&A code coexists with existing task/log/database APIs | `28 passed` | passed |
| `uv run pytest -q` | Full Python suite remains green after adding new models/routes/frontend contracts | `113 passed, 1 warning` | passed |
| `cd frontend && npm run build` | React/TypeScript build succeeds after task-detail composer split | Build succeeded | passed |
| `just docs-build` | MkDocs remains green after sidecar Q&A documentation updates | Build succeeded | passed |

### Errors
| Error | Resolution |
|-------|------------|
| Existing staged task-QA config/doc changes still treated `codex` as the default backend | Realigned the config, docs, and tests to `TASK_QA_BACKEND=chat_model`, which matches FR-9 and the implemented service |

## Session: 2026-03-26 Independent Task Q&A Sidecar PRD
...
### Actions Taken
- Read `/prd` and `planning-with-files` skill instructions and initialized `.claude/planning/current/`.
- Scanned the current task flow implementation in `dsl/api/tasks.py`, `dsl/services/codex_runner.py`, `dsl/services/task_service.py`, and `frontend/src/App.tsx`.
- Verified the existing bottom composer is execution-coupled: it persists `DevLog`, can regenerate PRD in `prd_waiting_confirmation`, and can interpret “continue/resume” text as workflow commands.
- Verified that task `DevLog` history is reused as automation prompt context, which makes it an unsafe persistence surface for “do not affect coding” Q&A.
- Read `ai_agent/utils/model_loader.py` and `docs/core/ai-assets.md`, then selected the side-path chat-model layer as the preferred implementation anchor for independent Q&A.
- Confirmed this requirement needs a prototype artifact because the behavior hinges on stage-aware UI interaction and execution isolation, not only static text.
- Added `docs/prototypes/task-sidecar-qa-demo.html` to demonstrate stage switching, independent question sending, and unchanged main execution state.
- Updated `mkdocs.yml` to expose both the existing workflow prototype and the new independent Q&A prototype in navigation.
- Authored the final PRD at `tasks/prd-514e2c11.md`, including metadata, clarifying questions, change matrix, diagrams, ER model, and actual prototype change log.
- Archived the planning session snapshot to `.claude/planning/sessions/20260326-114924-independent-task-q-a-sidecar`.

### Test Results
| Test | Expected | Actual | Status |
|------|----------|--------|--------|
| Discovery scan only | Establish current coupling points and reusable AI assets before authoring the PRD | Completed | passed |
| `just docs-build` | Prototype page, nav entry, and existing docs still build successfully | `Documentation built in 3.78 seconds` | passed |
...
## Session: 2026-03-26 Sidecar Q&A Round-2 Blocker Fixes
...
### Actions Taken
- Re-read the two blocking self-review findings and limited the fix scope to archived formal-feedback enforcement plus atomic single-pending sidecar Q&A protection.
- Added backend validation in `dsl/services/log_service.py` so `CLOSED` / `DELETED` tasks reject formal feedback writes even when callers bypass the frontend; media upload feedback inherits the same guard through the shared service path.
- Added a SQLite-only unique partial index for `task_qa_messages` creation time via model DDL and taught `utils/database.py` bootstrap to repair pre-existing duplicate pending assistant rows before recreating that index on older databases.
- Updated `dsl/services/task_qa_service.py` so question creation can hold a task-row lock on non-SQLite databases, avoid mid-flow commits when releasing stale pending replies, and translate `IntegrityError` races back into the existing pending-conflict `ValueError`.
- Added regression coverage for archived-task feedback rejection, SQLite duplicate-pending repair/bootstrap, the database-level single-pending invariant, and service-level translation of pending uniqueness conflicts.
- Synchronized the narrow system-design, evaluation, and PRD notes with the new backend and database guarantees.

### Test Results
| Test | Expected | Actual | Status |
|------|----------|--------|--------|
| `python -m py_compile dsl/services/task_qa_service.py dsl/services/log_service.py dsl/models/task_qa_message.py utils/database.py tests/test_task_qa_api.py tests/test_logs_api.py tests/test_database.py` | Edited backend files and regressions compile cleanly | Passed | passed |
| `uv run pytest tests/test_task_qa_api.py tests/test_logs_api.py tests/test_database.py -q` | Blocker-focused regressions pass | `22 passed` | passed |
| `uv run pytest tests/test_task_qa_api.py tests/test_task_qa_config.py tests/test_logs_api.py tests/test_database.py -q` | Sidecar + log/database regressions remain green with config coverage included | `27 passed` | passed |
| `just docs-build` | Documentation changes build successfully | Passed | passed |
| `git diff --check` | No whitespace or patch-format issues remain | Passed | passed |

### Errors
| Error | Resolution |
|-------|------------|
| The initial concurrency test tried to synchronize on `Session.commit()`, but SQLite had already serialized writers earlier at `flush()` and the test deadlocked | Replaced it with two stable regressions: one for the database-level pending uniqueness invariant and one for the service-layer `IntegrityError` to `ValueError` translation |
