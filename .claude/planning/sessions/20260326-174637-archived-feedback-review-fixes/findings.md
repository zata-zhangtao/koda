...
## 2026-03-26 Archived Feedback Review Fix Findings
- The archived-task formal-feedback guard in `LogService` was correct for public feedback, but the frontend still reused that same public path for three internal archive audit logs after the task had already become `DONE/CLOSED/DELETED`.
- The narrowest safe fix was to keep public formal-feedback rejection intact while adding a backend-only internal log path used exclusively by archive transitions.
- Pushing the archive audit logs down into the task API routes removes the UI's half-success state because the frontend no longer performs a second request that is guaranteed to be rejected after archival.
- Archived media uploads only needed cleanup, not a permission carve-out: the product rule is still that archived tasks reject new formal feedback, so the correct end state is “request failed and no file remained on disk”.
- Focused regressions in `tests/test_tasks_api.py` and `tests/test_media_api.py` are sufficient to cover the three broken archive flows and the two orphan-media paths called out by review.
