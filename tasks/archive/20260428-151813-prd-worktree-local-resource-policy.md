...
- FR-13: `Task.worktree_path` must be persisted only after Git worktree creation, resource materialization and bootstrap all succeed.
- FR-14: Materialization/bootstrap failure must attempt to remove the created worktree and new task branch before returning an error.
- FR-15: The shell bootstrap must not independently materialize local resources when Project policy is active.
- FR-16: Existing worktree path naming under `<repo-parent>/task/<repo>-wt-<task_short_id>` must not change.
- FR-17: Existing branch naming and base branch behavior must not change.
- FR-18: WebDAV business sync must exclude `worktree_resource_policy_json` and local resource contents.
- FR-19: Resource policy behavior must be covered by backend tests for confirmation, default classification, link, copy, skip, collision, path safety, rollback, WebDAV exclusion and legacy null-policy behavior.
- FR-20: Documentation must warn that linked resources are shared mutable state across worktrees.
