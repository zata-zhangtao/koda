...
### 7.9 Test Maintenance Update（2026-04-26）

- `just test` 暴露的 `test_app_exposes_abandon_action_for_abandoned_status` 失败不是功能回归：`frontend/src/App.tsx` 仍然保留 `Abandon` handler、日志写入和 `TaskLifecycleStatus.ABANDONED` 状态更新。
- 根因是静态源码断言要求 `taskApi.updateStatus(taskItem.id, TaskLifecycleStatus.ABANDONED)` 必须出现在同一行，无法容忍正常的格式化换行。
- 已将该断言改为等价的正则匹配，继续验证状态更新语义，同时不绑定具体换行格式。
- 验证结果：
  - `uv run pytest tests/test_project_timeline_api.py::test_app_exposes_abandon_action_for_abandoned_status -q` -> `1 passed`
  - `just test` -> `301 passed`
