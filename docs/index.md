...
- `WorkflowStage` 中的 `pr_preparing` 现在已接入真实自动化：`Complete` 会先进入该阶段，成功后自动推进到 `done`；前端会立即写回 `/complete` 返回的 `pr_preparing` 快照，并继续轮询 open 的 `pr_preparing`，直到自动刷新到最终 `done / CLOSED`。如果任务掉回 `changes_requested`，用户在 worktree 中修复实现、自检、lint 或 Git 环境问题后也可以直接再次点击 `Complete` 重试收尾，而不必重跑实现链。`test_in_progress` 现在承载 post-review lint / lint-fix，`acceptance_in_progress` 仍主要是预留阶段。
