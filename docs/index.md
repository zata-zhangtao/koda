...
- 默认不会在实现阶段自动执行 `git commit`；只有拥有 `worktree_path` 的任务才能展示普通 `Complete`。用户点击后，前端会先展示后端 canonical 的完成 checklist（最多 5 项，含 PRD acceptance 摘要与系统安全项），所有展示项勾选后才会提交 `/complete`。校验通过后，系统才会在任务 worktree 中执行 `git add .`，如果 staging 后仍有变更，就由当前 AI runner 基于 staged diff 生成符合 Conventional Commits 的 message 并提交；如果用户已经提交过导致 worktree 干净，则跳过 commit，随后执行 `git rebase <worktree_base_branch_name>`。新建/编辑任务时可从绑定项目的本地分支中选择 worktree 基底分支，默认仍为 `main`；同步该基底分支时会优先解析它配置的 remote，而不是写死 `origin`；merge 成功后的 cleanup 也会继续核验 worktree / branch 是否真的移除，并在需要时做 fallback 清理，最后再标记完成。
- `WorkflowStage` 中的 `pr_preparing` 现在已接入真实自动化：checklist gate 通过后，`Complete` 会先进入该阶段，成功后自动推进到 `done`；前端会立即写回 `/complete` 返回的 `pr_preparing` 快照，并继续轮询 open 的 `pr_preparing`，直到自动刷新到最终 `done / CLOSED`。如果任务掉回 `changes_requested`，用户在 worktree 中修复实现、自检、lint 或 Git 环境问题后也可以再次打开 checklist 并点击 `Complete` 重试收尾，而不必重跑实现链。`test_in_progress` 现在承载 post-review lint / lint-fix，`acceptance_in_progress` 仍主要是预留阶段。
- 对于“任务分支已被人工 merge 并删除”的边缘场景，后端会通过 `branch_health` 派生状态把已进入 worktree-backed Git 流程的任务标记为“缺失分支待确认”；前端要求用户打开同一类 checklist 并逐项确认，再走独立的 `/manual-complete` 收口接口，避免 backlog/未启动任务被误关单，也避免错误重跑普通 Git 收尾。
- `ai_agent/` 当前是工具库，不是 DSL 请求链路中的主处理器。

当前仍保留两个明确的人工边界：

- PRD 就绪后是否进入编码执行，仍需要用户确认；PRD 就绪来源可以是 AI 生成、`tasks/pending` 选择或手动导入（上传 / 粘贴 Markdown）
- self-review 闭环通过后是否执行最终 `Complete`，仍需要用户明确点击并完成 checklist 确认；若 AI 自检尚未形成最近一轮“通过”结论但后台已经空闲，用户也可以手动点击 `Complete`，系统会在 `DevLog` 中记录 checklist confirmation 与这次人工接管
